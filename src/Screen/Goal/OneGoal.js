...
const OneGoal = ({ route }) => {
  ...
  const handleDelete = (itemId) => {
    const goalRef = ref(db, 'goal/' + itemId);
    remove(goalRef)
      .then(() => {
        console.log('Goal deleted successfully');
        navigation.navigate('AllGoal'); // Navigate back to the AllGoal screen after deleting the goal
      })
      .catch((error) => {
  ...
  const handleUpdate = (item) => {
    navigation.navigate('UpdateGoal', { item });
  };
